...
YTDL_YOUTUBE_PLAYER_CLIENTS = os.getenv("YTDL_YOUTUBE_PLAYER_CLIENTS", "default")
...
async def dashboard_login(request: web.Request) -> web.Response:
    return web.json_response({"ok": False, "error": "Password login is disabled. Use Discord login."}, status=403)
...
async def start_dashboard() -> None:
    ...
    app.router.add_static("/static/", Path(__file__).with_name("static"))
...
def load_dashboard_login_html() -> str:
...
def main() -> None:
    ...
    register_music_commands(globals())
